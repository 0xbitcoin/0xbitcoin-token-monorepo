...
const same = async (
  ...
  const apy = options?.apy ?? 1000
  const paymentCycle = options?.paymentCycle ?? moment.duration(30, 'days')
  const dataHash = options?.dataHash ?? ethers.utils.keccak256('0x00000000')
